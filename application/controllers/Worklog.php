...
class Worklog extends CI_Controller {
	...
	public function __construct(){
		...
		$this->worklog_lists_fields = 'SQL_CALC_FOUND_ROWS ';

		$this->contract_update_fields = '';

		$this->current = 'worklog';//nevigation
	}

	/**
	* Purpose: Work Log list
	...
	*/
	public function lists()
	{
		...
		$result = $this->worklog_model->lists($this->worklog_lists_fields, $where, $like, $json = true, $orderby = '', $page, $pagesize);
	...
	*/
	public function update(){
		...
		if($contract_id){
			$result = $this->worklog_model->lists($this->contract_update_fields, '', '' , $json = true, $orderby = '', 1, 1);
			$data['contract'] = $result['result'];
			$data['current'] = $this->current;
	...
	/**
	* Purpose: Next ticket number ajax
	* @param null
	* @return: {json}
	*/
	public function nextTicket(){
		$current_ticket = $this->worklog_model->nextTicket();
		echo json_encode($current_ticket);
	}
