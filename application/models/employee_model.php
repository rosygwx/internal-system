...
class Employee_model extends CI_Model{
	...
	public function lists( $fields = '*' , $where = '', $like = '', $json = true, $orderby = array('employee_id'=>'desc'), $page = 1, $pagesize = 500, $like_query ='' ){
		if( is_array( $fields ) ) {
			$fld = implode( ',' , $fields );
		}else
			$fld = $fields;
		...
		if(is_array( $like_query )){
			foreach ( $like_query as $key => $value ) {
				$this->db->where( $value );
			}
		}
		...
		$query = $this->db->get( $this->employee_table , $pagesize , ($page-1) * $pagesize);
